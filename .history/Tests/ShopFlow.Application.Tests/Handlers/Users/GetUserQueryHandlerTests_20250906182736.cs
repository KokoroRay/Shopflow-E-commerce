...
public class GetUserQueryHandlerTests
{
    ...
    public async Task Handle_UserWithDifferentStatuses_ShouldReturnCorrectStatus(UserStatus status, byte expectedStatusByte)
    {
        ...
        // Note: UserStatus and EmailVerified are read-only properties set in constructor
        // We can only test with default values from constructor
        _userRepositoryMock.Setup(x => x.GetByIdAsync(userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(user);
        ...
        // Assert
        result.Should().NotBeNull();
        result!.Status.Should().Be((byte)UserStatus.Active); // Always Active from constructor
    }
    ...
    public async Task Handle_UserWithDifferentEmailVerificationStatus_ShouldReturnCorrectVerification(bool emailVerified)
    {
        ...
        // Note: EmailVerified is read-only property set in constructor to false
        _userRepositoryMock.Setup(x => x.GetByIdAsync(userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(user);
        ...
        // Assert
        result.Should().NotBeNull();
        result!.EmailVerified.Should().BeFalse(); // Always false from constructor
    }
