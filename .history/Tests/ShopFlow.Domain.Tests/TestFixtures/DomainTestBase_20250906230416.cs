...
public abstract class DomainTestBase
{
    ...
    private void ConfigureEntityCustomizations()
    {
        ...
        // Configure CatProduct creation
        Fixture.Customize<CatProduct>(composer => composer
            .Without(p => p.Skus)
            .Without(p => p.Reviews)
            .Without(p => p.Categories)
            .With(p => p.ProductType, () => (byte)1)
            .With(p => p.Status, () => ProductStatus.Draft));

        // Configure CoreAddress creation
        Fixture.Customize<CoreAddress>(composer => composer
            .Without(a => a.User)
            .With(a => a.Street, () => Fixture.Create<string>())
            .With(a => a.City, () => Fixture.Create<string>())
