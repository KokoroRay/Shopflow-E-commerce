...
public class PhoneNumberTests : DomainTestBase
{
    ...
    [InlineData("")]
    [InlineData(" ")]
    public void Constructor_EmptyOrWhitespacePhone_ShouldThrowArgumentException(string invalidPhone)
    {
        // Act & Assert
    ...
    [Fact]
    public void Constructor_NullPhone_ShouldThrowArgumentException()
    {
        // Act & Assert
        AssertThrows<ArgumentException>(() => new PhoneNumber(null!), "Phone number cannot be empty");
    }
    ...
    public void Constructor_PhoneWithSpecialCharacters_ShouldThrowArgumentException(string phoneWithSpecialChars)
    {
        // Act & Assert
        // Current implementation validates format first, so special characters should be rejected
        AssertThrows<ArgumentException>(() => new PhoneNumber(phoneWithSpecialChars), "Invalid phone number format");
    }
    ...
    {
        // Arrange - Complex phone should be rejected due to special characters
        var complexPhone = "+84 (0) 90-123-4567";

        // Act & Assert
        AssertThrows<ArgumentException>(() => new PhoneNumber(complexPhone), "Invalid phone number format");
    }

    [Theory]
    [InlineData("0001234567")] // Invalid prefix - starts with 0
