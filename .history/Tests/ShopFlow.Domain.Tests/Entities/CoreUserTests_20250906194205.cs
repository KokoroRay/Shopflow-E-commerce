...
public class CoreUserTests : DomainTestBase
{
    ...
    [Fact]
    public void Constructor_EmptyPasswordHash_ShouldThrowException()
    {
        // Act & Assert
        var exception = Assert.Throws<ArgumentNullException>(() => 
            new CoreUser(_validEmail, "", _validPhone));
    ...
    public void UpdatePhone_ValidPhone_ShouldUpdatePhone()
    {
        // Arrange
        var user = new CoreUser(_validEmail, _validPasswordHash);
        var newPhone = new PhoneNumber("9876543210");
        var originalUpdatedAt = user.UpdatedAt;
    ...
    public void HasRole_MultipleRoles_ShouldReturnCorrectResult()
    {
        ...
        var adminRole = Fixture.Build<Role>()
            .With(r => r.Code, "ADMIN")
            .Create();
        var userRole = Fixture.Build<Role>()
            .With(r => r.Code, "USER")
            .Create();
            
        var adminUserRole = Fixture.Build<CoreUserRole>()
            .With(ur => ur.Role, adminRole)
            .Create();
        var normalUserRole = Fixture.Build<CoreUserRole>()
            .With(ur => ur.Role, userRole)
            .Create();
    ...
    public void UserLifecycle_CompleteFlow_ShouldWorkCorrectly()
    {
        ...
        var role = Fixture.Build<Role>()
            .With(r => r.Code, "CUSTOMER")
            .Create();
        var userRole = Fixture.Build<CoreUserRole>()
